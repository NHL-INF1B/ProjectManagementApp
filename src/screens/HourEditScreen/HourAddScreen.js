...
//function to plan notifications.
...
const HourAddScreen = () => {
  ...
  //when the page opens.
  useEffect(() => {
    //get the permission to send notifications.
  ...
  // Add points when the user adds a new hour.
  ...
  //the regex to check the date and time.
  const DATE_REGEX = /^\d{4}-\d{2}-\d{2}$/;
  const TIME_REGEX = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/;

  //When the button is pressed.
  const submitData = (data) => {
    //plan a new notification, add points and send input data to API.
    planNotification();
    addPoints(userId, projectId);
    sendDataToAPI(data);
  };

  // Inserting the data into the database and catching the feedback.
